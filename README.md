...
VotingEngine implements the core voting functionality in ServicesDAO. Repository contains two application for reputation management and voting process.<br><br>
...
```
Docker-compose downloads/creates(if necessary) builds and runs;<br>
...
Due to its microservice architecture, the voting engine comminucates with the reputation service internally thru address "http://dao_reputationservice".<br>
...
dotnet test --filter DisplayName~
```
Code documentation files in format is autogenerated everytime the project is build under bin folder.<br>
...
```
https://+ is added to ASPNETCORE_URLS and 443 port is defined as https port with adding ASPNETCORE_HTTPS_PORT=443<br><br>
